...
function BasePlayer:Register(password)
    ...
    if not self.hasAccount then
        ...
        tes3mp.MessageBox(self.pid, -1, "Дальше вы не пройдете пока не получите бумаги!")    
        tes3mp.PlaySpeech(self.pid, "Vo\\Misc\\CharGen Door1.mp3")    
    end
end
...
function BasePlayer:FinishLogin()
    ...
    if self.hasAccount ~= false then -- load account
        startupScripts.OnLogin(self.pid)
        self:SaveIpAddress()
